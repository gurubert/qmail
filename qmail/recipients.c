...
int len;
{
  ...
  if (at < len) {
    ...
    if (!stralloc_cats(&address,&domain)) return -2; 
  }
...
int recipients_parse (rhost,rlen,addr,rkey,klen,vaddr,vkey,vlen)
char *rhost;
int rlen;
char *addr;
char *rkey;
